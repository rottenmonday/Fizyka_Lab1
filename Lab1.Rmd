...
Średnie długości wysokości, boków a oraz b prostopadłościanu wynoszą odpowiednio `r prost_h_avg`mm, `r prost_a_avg`mm, `r prost_b_avg`mm. Z otrzymanych wyników otrzymano objętość prostopadłościanu V = `r prost_V`mm^3^.

Wysokość prostopadłościanu mierzono za pomocą mikrometru, którego niepewność wzorcowania wynosi 0.01mm. Niepewność eksperymentatora przyjęto za 0.005mm. Niepewność typu B dla wysokości wynosi więc $\sqrt{\frac{0.01^2}{3} + \frac{0.005^2}{3}}$mm, czyli `r prost_B_h`cm. Ze wzoru $\sqrt{\frac{\sum_{i=1}^N(x_i-x_s)^2}{N*(N-1)}}$ niepewność typu A wynosi `r prost_A_h`mm, co daje niepewność całkowitą równą `r h_cal`mm.
...
Aby obliczyć niepewność całkowitą skorzystamy ze wzoru  $u(V)=\sqrt{(\frac{\partial V}{\partial h})u^2(h) + (\frac{\partial V}{\partial a})u^2(a) + (\frac{\partial V}{\partial b})u^2(b) }$. Niepewność złożona objętości wynosi: `r prost_cal`cm^3^. Co daje ostateczny wynik `r prost_V`(`r prost_cal`)cm^3^.
...
Korzystając ze wzoru na objętość walca z wyciętym kawałkiem: $V = \frac{\pi}{4}(D^2H - d^2h)$, policzymy objętość walca z laboratorium. Następnie wyliczymy niepewność pomiarową tych obliczeń, korzystając ze wzoru na niepewność złożoną. W pomiarach na D, H i d bierzemy pod uwagę tylko niepewność typu B, natomiast w pomiarze h bierzemy pod uwagę oba typy niepewności
...
Policzymy teraz niepewność typu A dla wysokości walca wewnętrznego. Wyraża się ona wzorem: $\sqrt{\frac{\sum_{i=1}^N(x_i-x_s)^2}{N*(N-1)}}$. Niepewność ta wynosi: `r aniepval`cm. Niepewność całkowita dla wysokości walca wewnętrznego wynosi: `r niepcal`cm.

Korzystając ze wcześniej wspomnianego wzoru i odpowiednich wartości wyliczamy, że objętość walca wynosi: `r V`$cm^3$. Skorzystamy teraz ze wzoru na niepewność złożoną aby wyliczyć niepewność obliczania objętości walca: $u(V)=\sqrt{(\frac{\partial V}{\partial D})u^2(D) + (\frac{\partial V}{\partial d})u^2(d) + (\frac{\partial V}{\partial H})u^2(H) + (\frac{\partial V}{\partial h})u^2(h)}$. Niepewność złożona wynosi ostatecznie: `r sumbledroot`cm^3^. W takim razie wyliczona objętość wynosi `r V`(`r sumbledroot`)$cm^3$.
...
Spróbujemy, na podstawie obliczeń na oporniku R4, wyznaczyć opór tego opornika. Badaliśmy napięcia i natężenia z różnymi zakresami. Zobaczmy najpierw wszystkie dane na wykresie:
...
Widzimy, że jedna obserwacja zdecydowanie nie pasuje do reszty i zostanie potraktowana jako błąd gruby. Przeprowadzimy teraz prostą przez zbiór obserwacji.
...
Z dopasowania prostej do zestawu danych widzimy, że średni opór wynosi $383.587 \Omega$. Jest to współczynnik kierunkowy tej prostej. Niepewność tych obliczeń dopasowania wynosi $2.639$. Opór wynosi więc $383.58(2.64)\Omega$

Nastepnie przedstawiamy wyliczone dla każdej obserwacji wartości oporu, wraz z niepewnością złożoną. Niepewność złożona dla oporu liczona jest ze wzoru: $u_R = \sqrt{(\frac{\partial R}{\partial U})u_U^2 + (\frac{\partial R}{\partial I})u_I^2}$.
Wartość $u_U$ wyliczana jest ze wzoru $u_U = \frac{\Delta U}{\sqrt{3}}$, gdzie $\Delta U=\frac{klasa*zakres}{100}$, natomiast wartość $u_I$ ze wzoru $u_I=\frac{\Delta I}{\sqrt{3}}$, gdzie $\Delta I=0.012 I +0.1$ dla zakresu $200mA$, natomiast $\Delta I=0.005 I + 0.01$ dla zakresu $20mA$.
...
Podczas zajęć wykonano pomiary niezbędne do wyznaczenia objętości metalowego prostopadłościanu, bryły w kształcie walca z wyciętym walcem oraz pomiary napięcia i natężenia czterech oporników, które posłużyły do wyznaczenia oporów wraz z towarzyszącymi im błędami pomiarowymi. Na podstawie pomiarów wykonanych dla opornika nr 4 wyznaczono wykres prądowo-napięciowy, wyraźnie ukazujący liniową zależność napięcia od natężenia, co potwierdza prawo Ohma. 
...
Różne wyniki przy pomiarach prostopadłościanu i drugiej bryły mogły się brać z błędów przy odczytywaniu wyniku z aparatury, za mocnym dociśnięciem suwmiarki, jak również z uszczerbków i wypukłości na powierzchni brył.
